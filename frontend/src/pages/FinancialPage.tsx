...
export function FinancialPage() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      incomeCount: incomeTrans.length
    };
  ...
  const monthlyData = [
    ...
    { month: 'Mai', value: summary.income || 55000 },
  ];
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <motion.div variants={itemVariants}>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">Gestão <span className="text-primary-600">Financeira</span></h1>
        ...
        <motion.div variants={itemVariants} className="flex gap-3">
           ...
           >
            <Download size={18} className="text-slate-400" /> Exportar Relatório
          </button>
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <motion.div variants={itemVariants} className="md:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm flex flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-8">
               ...
               <span className="text-[10px] font-bold text-slate-400">Mensal / 2026</span>
            </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <motion.div variants={itemVariants} className="bg-slate-900 rounded-[2.5rem] p-6 text-white shadow-xl relative overflow-hidden group">
            ...
            <h3 className="text-3xl font-black tracking-tight">R$ {Number(metrics.ticketMedio).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</h3>
            <div className="flex items-center gap-2 mt-4 text-[10px] text-slate-400 font-bold uppercase">
              <Receipt size={12} /> {metrics.incomeCount} OS Faturadas
            </div>
          ...
          <motion.div variants={itemVariants} className="bg-white rounded-[2.5rem] p-6 border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <div className="flex items-end gap-2">
              ...
              <span className="text-emerald-500 font-bold text-xs mb-1 flex items-center gap-0.5">
            ...
            <div className="mt-4 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${metrics.lucratividade}%` }}
        ...
        <motion.div variants={itemVariants} className="bg-primary-600 rounded-[2.5rem] p-8 text-white shadow-xl shadow-primary-500/20 flex flex-col justify-between relative overflow-hidden">
           <div className="relative z-10">
             ...
             <h3 className="text-4xl font-black tracking-tighter">R$ {Number(summary.balance).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</h3>
             <div className="mt-6 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center"><CheckCircle2 size={16} /></div>
                <p className="text-[10px] font-bold text-primary-100 uppercase tracking-widest">Saúde Financeira: ÓTIMA</p>
             </div>
           </div>
           <button className="w-full py-4 bg-white text-primary-600 rounded-2xl text-xs font-black uppercase tracking-widest transition-all hover:bg-primary-50 relative z-10">Ver Detalhes do Saldo</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div variants={itemVariants} className="bg-emerald-50 border border-emerald-100 p-6 rounded-[2rem] flex items-center justify-between group">
           <div>
             ...
             <h4 className="text-2xl font-black text-slate-900">R$ {Number(summary.income).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h4>
           </div>
        ...
        <motion.div variants={itemVariants} className="bg-red-50 border border-red-100 p-6 rounded-[2rem] flex items-center justify-between group">
           <div>
             ...
             <h4 className="text-2xl font-black text-slate-900">R$ {Number(summary.expense).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h4>
           </div>
        ...
        <motion.div variants={itemVariants} className="bg-slate-50 border border-slate-200 p-6 rounded-[2rem] flex items-center justify-between group">
           <div>
             ...
             <h4 className="text-2xl font-black text-slate-900">R$ {Number(summary.income * 1.1).toLocaleString('pt-BR', { minimumFractionDigits: 0 })}</h4>
           </div>
      ...
      <motion.div variants={itemVariants} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-50/20">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
             ...
             <button className="p-3.5 border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all text-slate-400"><Filter size={20} /></button>
          </div>
        </div>
                    ...
                    <td className="px-8 py-6">
                      ...
                      {t.referenceId && (
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className="px-2 py-0.5 bg-primary-50 text-primary-600 rounded-md text-[9px] font-black uppercase tracking-tighter">Vínculo OS: {t.referenceId.slice(0, 8)}</span>
                        </div>
                      )}
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                ...
                <button 
                  onClick={() => setShowAddModal(false)}
                  className="p-3 hover:bg-slate-200 rounded-2xl text-slate-400 hover:text-slate-900 transition-colors"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleAddTransaction} className="p-10 space-y-6">
                <div className="grid grid-cols-2 gap-4 p-1 bg-slate-100 rounded-2xl mb-6">
                  <button
                    ...
                    className={cn(
                      ...
                      formData.type === 'INCOME' ? "bg-emerald-500 text-white shadow-lg" : "text-slate-500 hover:bg-white/50"
                    )}
                  >
                    Entrada
                  </button>
                  <button
                    ...
                    className={cn(
                      "py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                      formData.type === 'EXPENSE' ? "bg-red-500 text-white shadow-lg" : "text-slate-500 hover:bg-white/50"
                    )}
                  >
                    Saída
                  </button>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-slate-900/5 focus:border-slate-900 transition-all font-bold text-sm"
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-slate-900/5 focus:border-slate-900 transition-all font-black text-lg"
                    />
                  </div>
                  <div className="space-y-1.5">
                ...
                <div className="pt-6">
                  <button
                    type="submit"
                    className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-xs shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95"
                  >
